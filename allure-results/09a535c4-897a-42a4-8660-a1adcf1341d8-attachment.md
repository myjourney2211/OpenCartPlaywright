...
```ts
  ...
  16 | let letcheckOutPg: checkOutPage;
  ...
  29 |     letcheckOutPg = new checkOutPage(page);
  ...
  82 | 
> 83 |     expect(await letcheckOutPg.isCheckOutPageAvailableByTitle()).toBe(true);
     |                                                                  ^ Error: expect(received).toBe(expected) // Object.is equality
  84 | 
  85 |     expect(await letcheckOutPg.isCheckOutPageAvailableByHeader()).toBeTruthy();
  86 | 
  87 | });
```