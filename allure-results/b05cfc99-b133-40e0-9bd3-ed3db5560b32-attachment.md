...
- Name: LoginDataDriven.spec.ts >> Login test with CSV Data : Invalid username and password @datadriven
- Location: tests\LoginDataDriven.spec.ts:40:9
        ...
        - generic [ref=e87]:
          ...
          - generic [ref=e91]:
            - generic [ref=e92]:
              ...
              - textbox "E-Mail Address" [ref=e94]: testagent@test.comttt
            - generic [ref=e95]:
              - generic [ref=e96]: Password
              - textbox "Password" [ref=e97]: Password@12345
...
```ts
  ...
  7  | 
  8  | //Load JSON test data
  9  | const jsonpath = "testdata/loginData.json";
  10 | const jsontestdata: any = dataProvider.getTestDataFromJson(jsonpath);
  11 | 
  ...
  19 |         await home.clickMyAccount();
  20 |         await home.clickLogin();
  21 | 
  22 |         const loginPG = new loginPage(page);
  23 |         await loginPG.login(data.email, data.password);
  24 | 
  25 |         if (data.expected.toLowerCase() === "success") {
  26 |             const accPG = new myAccountPage(page);
  27 |             expect(await accPG.myAccountPageVisibility()).toBeTruthy();
  28 |         } else {
  29 |             const erroMessage = await loginPG.getLoginErrorMessage();
  30 |             expect(erroMessage).toBe('Warning: No match for E-Mail Address and/or Password.')
  31 |         }
  32 |     })
  33 | }
  34 | 
  35 | //Load CSV test data
  36 | const csvpath = "testdata/loginData.csv";
  37 | const csvtestdata: any = dataProvider.getTestDataFromCSV(csvpath);
  38 | 
  39 | for (const data of csvtestdata) {
  40 |     test(`Login test with CSV Data : ${data.testname} @datadriven`, async ({ page }) => {
  41 | 
  42 |         const config = new TestConfig();
  43 |         await page.goto(config.appUrl);
  44 | 
  45 |         const home = new homePage(page);
  46 |         await home.clickMyAccount();
  47 |         await home.clickLogin();
  48 | 
  49 |         const loginPG = new loginPage(page);
  50 |         await loginPG.login(data.email, data.password);
  51 | 
  52 |         if (data.expected.toLowerCase() === "success") {
  53 |             const accPG = new myAccountPage(page);
  54 |             expect(await accPG.myAccountPageVisibility()).toBeTruthy();
  55 |         } else {
  56 |             const erroMessage = await loginPG.getLoginErrorMessage();
> 57 |             expect(erroMessage).toBe('Warning: No match for E-Mail Address and/or Password.')
     |                                 ^ Error: expect(received).toBe(expected) // Object.is equality
  58 |         }
  59 |     })
  60 | }
